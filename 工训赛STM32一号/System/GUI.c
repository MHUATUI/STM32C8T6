...
{  
	...
	LCD_SetWindows(x,y,x+32-1,y+64-1);//设置单个文字显示窗口
	
	for(index = 0; index < size; index++) {
		temp = asc2_6432[num][index];
		for(t=0;t<8;t++){                 
...
//			if(size==76){
//				temp=asc2_6432[num][pos];			//调用6432字体
//			}
//				for(t=0;t<(size - 12)/2;t++){                 
//					if(temp&0x01)Lcd_WriteData_16Bit(fc); 
//					else Lcd_WriteData_16Bit(bc); 
//					temp>>=1; 
//				}
...
int LCD_ShowNum0x0(u8 num1, u8 num2, u8 num3, u8 num4, u8 num5, u8 num6){
	if(num6) {
		LCD_ShowChar(0,0,POINT_COLOR,BACK_COLOR,num1,180,0);
		LCD_ShowChar(32,0,POINT_COLOR,BACK_COLOR,num2,180,0);
		LCD_ShowChar(64,0,POINT_COLOR,BACK_COLOR,num3,180,0);
		LCD_ShowChar(0,45,POINT_COLOR,BACK_COLOR,4,180,0);
		LCD_ShowChar(0,85,POINT_COLOR,BACK_COLOR,num4,180,0);
		LCD_ShowChar(32,85,POINT_COLOR,BACK_COLOR,num5,180,0);
		LCD_ShowChar(64,85,POINT_COLOR,BACK_COLOR,num6,180,0);
		return 1;
	}
	return 0;
}
